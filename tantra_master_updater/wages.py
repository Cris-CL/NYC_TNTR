...
def update_wages(sheet_name):
  if sheet_name == 'MASTER':
    ...
    try:
        extra_sh = gc.open_by_key(WAGES_SHEET_ID)
        ws = extra_sh.worksheet(sheet_name)

        df =pd.DataFrame(ws.get_all_values())
        df = df.iloc[1:]
        df = df[df.columns[0:7]]
        df.columns = ["hostess_name","real_name","wage_class","cp_code","hour_a","hour_b","YEAR_MONTH"]
        ### Clean the data
        for col in integer_cols.keys():
            df[col] = df[col].map(lambda x: x.replace(",","").replace(" ","").replace("(","-").replace(")","").replace(".00","") if isinstance(x,str) else x)
        ...
        df.to_gbq(
        destination_table=f"{DATASET}.{HOSTESS_WAGE}_2",
